<template>
  <div class="hello">
    <h1>The joker joked {{ count }} times to the scary thing!</h1>
    <h2><router-link :to="{ name: 'JokeCounter', params: { count: countplusone }}">Joke Again?</router-link></h2>

    <h3 v-if="isCountEnough">Boo! has laughed so hard he vanished!</h3>
...
export default {
  ...
  computed: {
    ...
    isCountEnough: function () {
      var count = parseInt(this.count, 10)
      return count >= 100
    }
  }
}
